        ...
        foreach ($arrayParticipants as $key => $participant) {
            ...
            foreach ($arrayData as $data) {
                ...
                if (isset($data['participants']) && array_key_exists($key, $data['participants'])) {
                    echo '<td>'.yii\bootstrap\BaseHtml::checkbox('dateparticipant['.$dateCours.']['.$data['model']->cours_date_id.'|'.$key.']', true).'</td>';
                } else echo '<td>'.yii\bootstrap\BaseHtml::checkbox('dateparticipant['.$dateCours.']['.$data['model']->cours_date_id.'|'.$key.']', false).'</td>';
            }
            echo '</tr>';
        }
        echo '</table>';
