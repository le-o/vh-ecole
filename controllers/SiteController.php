...
//        $coursDateID = explode(',', '7447,7197,6613,7439,7582,7601,7619,7565,7628,7650,7509,7574,7602,7629,7568,7688,7630,7651,7519,7631,7632,7510,7645,7646,7652,7520,7647,7648,7653,7511,7512,7649,7521,7633,7522,7634,7513,7635,7523,7636,7637,7524,7638,7639,7518,7640,7641,7642,7525,7643,7644,7526,7527,7463,7468,7570,7542,7563,6411,7654,6412,7444,7569,7627,6413,6414,6415,6416,6417,6418,6419,6420,6421,6422,6423,6424,6425,6426,6427,7202,7219,7372,7236,7253,7270,7287,7603,7304,7321,7338,7389,7406,7203,7220,7373,7237,7254,7271,7288,7604,7305,7322,7339,7390,7404,7204,7221,7374,7238,7255,7272,7289,7605,7306,7323,7340,7599,7405,7205,7222,7375,7239,7256,7273,7290,7606,7307,7324,7341,7395,7407,7206,7223,7376,7240,7257,7274,7291,7607,7308,7325,7342,7396,7685,7207,7224,7377,7241,7258,7275,7292,7608,7309,7326,7343,7397,7686,7408,7209,7226,7379,7243,7260,7277,7294,7609,7311,7328,7345,7392,7210,7227,7380,7244,7261,7278,7295,7610,7312,7329,7346,7393,7211,7228,7381,7245,7262,7279,7296,7611,7313,7330,7347,7394,7216,7587,7378,7575,7242,7259,7293,7581,7612,7319,7576,7336,7417,7212,7229,7382,7246,7263,7280,7297,7613,7314,7331,7348,7401,7213,7230,7383,7247,7264,7281,7298,7614,7315,7332,7349,7402,7214,7231,7384,7248,7265,7282,7299,7615,7316,7333,7350,7413,7215,7232,7385,7249,7266,7283,7300,7616,7317,7334,7351,7414,7233,7592,7386,7250,7267,7284,7301,7617,7318,7335,7352,7415,7217,7234,7387,7251,7268,7285,7302,7618,7310,7588,7353,7409,7410,7411,7412');
        ...
        // modification heure anniversaire selon carte Trello (v7.0)
        /**
         * select cd.*
        from cours c
        join cours_date cd on c.cours_id = cd.fk_cours
        -- left join clients_has_cours_date ccd on cd.cours_date_id = ccd.fk_cours_date
        where c.fk_nom = 246
        and cd.date > '2022-01-09'
        -- and cd.cours_date_id not in (select distinct fk_cours_date from clients_has_cours_date)
        and cd.heure_debut = '13:45:00'
        -- and cd.cours_date_id > 15000
        order by cd.date;
         */
/**
        $query = CoursDate::find()
            ->where(['>', 'date', '2022-01-09'])
            ->andWhere(['=', 'heure_debut', '13:45:00'])
            ->andWhere(['=', 'cours.fk_nom', 246])
            ->orderBy('fk_lieu, date ASC');
        $query->joinWith(['fkCours']);
        $modelsCoursDate = $query->all();

        echo 'BEGIN <br />
        Traitement doublon 13:45:00 - nb ' . count($modelsCoursDate);

        $previous = new CoursDate();
        $previous->date = '2021-01-01';
        $i = 0;
        $transaction = \Yii::$app->db->beginTransaction();
        try {
            foreach ($modelsCoursDate as $modelCD) {
                if (strtotime($previous->date) !== strtotime($modelCD->date)) {
                    echo '<br /><br />* Traitement date ' . $modelCD->date;
                } else {
                    echo ' ! doublon détecté pour IDs [' . $previous->cours_date_id . ' - ' . $modelCD->cours_date_id . '] --> modification de l\'heure à 14:00 pour ' . $modelCD->cours_date_id;
                    $modelCD->heure_debut = '14:00';
                    $modelCD->save();
                    $i++;
                }
                $previous = $modelCD;
        ...
        Traitement doublon 13:45:00 - nb modifié ' . $i;


        $query = CoursDate::find()
            ->where(['>', 'date', '2022-01-09'])
            ->andWhere(['=', 'heure_debut', '16:15:00'])
            ->andWhere(['=', 'cours.fk_nom', 246])
            ->orderBy('fk_lieu, date ASC');
        $query->joinWith(['fkCours']);
        $modelsCoursDate = $query->all();

        echo '<br /><br /><br />**********--************--*********<br /><br />BEGIN<br />
        Traitement doublon 16:15:00 - nb ' . count($modelsCoursDate);
        ...
        try {
            foreach ($modelsCoursDate as $modelCD) {
                if (strtotime($previous->date) !== strtotime($modelCD->date)) {
                    echo '<br /><br />* Traitement date ' . $modelCD->date;
                    echo ' --> modification de l\'heure à 15:45 pour ' . $modelCD->cours_date_id;
                    $modelCD->heure_debut = '15:45';
                    $modelCD->save();
                } else {
                    echo ' ! doublon détecté pour IDs [' . $previous->cours_date_id . ' - ' . $modelCD->cours_date_id . '] --> modification de l\'heure à 16:00 pour ' . $modelCD->cours_date_id;
                    $modelCD->heure_debut = '16:00';
                    $modelCD->save();
        ...
        echo '<br /><br />--------------<br />END<br />
        Traitement doublon 16:15:00 - nb modifié ' . $i;
